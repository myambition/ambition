...
func submain() int {
	...
	fsEmptyRPC := flag.NewFlagSet("emptyrpc", flag.ExitOnError)
	...
	case "emptyrpc":
		fsEmptyRPC.Parse(flag.Args()[1:])

		request, err := handlers.EmptyRPC()
		if err != nil {
			fmt.Fprintf(os.Stderr, "Error calling handlers.EmptyRPC: %v\n", err)
			return 1
		}

		v, err := service.EmptyRPC(ctx, request)
		if err != nil {
			fmt.Fprintf(os.Stderr, "Error calling service.EmptyRPC: %v\n", err)
			return 1
		}
		fmt.Println("Client Requested with:")
		fmt.Println()
